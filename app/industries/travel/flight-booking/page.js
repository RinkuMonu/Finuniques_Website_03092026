...
const features = [
  {
    icon: Send,
    title: "Flight Collections",
    description:
      "Collect payments for fares, seats, baggage and add-ons in one dependable booking flow.",
  },
  {
    icon: ArrowLeftRight,
    title: "Change and Refund Control",
    description:
      "Manage cancellations, reschedules and refunds with clear status for every traveler.",
  },
  {
    icon: BarChart3,
    title: "Air Travel Reporting",
    description:
      "Track bookings, payment performance and airline or partner settlements from one place.",
  },
];
...
const bookingSteps = [
  [
    "01",
    "Build the itinerary",
    "Let travelers choose flights, seats and extras without losing the booking context.",
  ],
  [
    "02",
    "Authorize the fare",
    "Accept secure payment and keep ticket confirmation aligned with payment status.",
  ],
  [
    "03",
    "Settle every partner",
    "Reconcile airlines, agents, refunds and ancillary collections with less manual work.",
  ],
];
...
export default function FlightBookingPage() {
  return (
    <main>
        ...
        <div className="relative mx-auto grid max-w-[1580px] items-center gap-12 px-6 py-20 sm:px-10 sm:py-28 lg:grid-cols-[1fr_.9fr] lg:px-16 lg:py-32">
          ...
          >
            ...
            <h1 className="mt-6 max-w-3xl text-5xl font-black leading-[1.02] tracking-[-0.04em] sm:text-6xl lg:text-7xl">
              Help every traveler take off with confidence.
            </h1>
            <p className="mt-7 max-w-2xl text-lg leading-8 text-white/70">
              Power flight reservations with reliable checkout, real-time
              payment status and flexible controls for tickets, changes and
              refunds.
            </p>
            <div className="mt-9 flex flex-col gap-3 sm:flex-row">
              <Link
                href="/contact"
                className="inline-flex items-center justify-center gap-2 rounded-full bg-white px-7 py-4 text-sm font-bold text-[#073B4C] transition hover:bg-cyan-100"
              >
          ...
          >
            ...
            <div className="relative rounded-[28px] border border-white/15 bg-white p-5 text-[#073B4C] shadow-[0_30px_80px_rgba(0,0,0,.24)]">
              <div className="flex items-center justify-between border-b border-slate-100 pb-5">
                <div>
                  <p className="text-xs font-black uppercase tracking-[.16em] text-[#0797D0]">
                    Live itinerary
                  </p>
                  <p className="mt-1 text-xl font-black">Delhi to Singapore</p>
                </div>
                <span className="grid h-12 w-12 place-items-center rounded-2xl bg-[#073B4C] text-cyan-300">
                  <Plane className="h-6 w-6" />
                </span>
              </div>
              <div className="mt-5 grid gap-3">
                <div className="flex items-center justify-between rounded-xl bg-[#EEF8FC] p-4">
              ...
              <div className="mt-5 flex items-center justify-center gap-2 rounded-xl bg-emerald-50 py-3 text-sm font-bold text-emerald-700">
                <CheckCircle2 className="h-4 w-4" /> Ticket payment confirmed
              </div>
            </div>
          </motion.div>
        </div>
      </section>
      <section className="border-b border-[#CFE5EE] bg-[#F7FCFE]">
        <div className="mx-auto grid max-w-[1580px] divide-y divide-[#CFE5EE] px-6 sm:grid-cols-3 sm:divide-x sm:divide-y-0 sm:px-10 lg:px-16">
          {metrics.map(([value, label, Icon]) => (
            <div
              key={label}
              className="flex items-center gap-4 py-6 sm:justify-center"
            >
              <Icon className="h-5 w-5 text-[#0797D0]" />
              <div>
                <p className="text-2xl font-black text-[#073B4C]">{value}</p>
                <p className="text-sm font-semibold text-[#527582]">{label}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="relative mx-auto max-w-[1580px] px-6 sm:px-10 lg:px-16">
          ...
          >
            ...
            <h2 className="text-4xl font-bold leading-[1.08] tracking-[-0.04em] text-[#073B4C] sm:text-5xl lg:text-6xl xl:text-[64px]">
              Built for complex air travel businesses.
            </h2>
          </motion.div>
      ...
      <section
        id="booking-flow"
        className="bg-[#447686] py-24 text-white lg:py-28"
      >
        <div className="mx-auto max-w-[1580px] px-6 sm:px-10 lg:px-16">
          <div className="max-w-2xl">
            ...
            <h2 className="mt-5 text-4xl font-black tracking-tight sm:text-5xl">
              A smoother journey from search to boarding.
            </h2>
          </div>
          <div className="mt-14 grid gap-5 md:grid-cols-3">
            {bookingSteps.map(([number, title, text]) => (
              <article
                key={number}
                className="rounded-[28px] border border-white/15 bg-white/5 p-7 sm:p-9"
              >
                <p className="text-5xl font-black text-cyan-300/50">{number}</p>
      ...
      <section className="bg-[#EEF8FC] py-20">
        <div className="mx-auto flex max-w-[1580px] flex-col justify-between gap-8 px-6 sm:px-10 md:flex-row md:items-center lg:px-16">
          <div>
            <p className="text-sm font-bold uppercase tracking-[.3em] text-[#0797D0]">
              Ready when you are
            </p>
            <h2 className="mt-4 max-w-2xl text-4xl font-black tracking-tight text-[#073B4C] sm:text-5xl">
              Make every flight booking count.
            </h2>
          </div>
          <Link
            href="/signup"
            className="inline-flex w-fit items-center gap-2 rounded-full bg-[#0797D0] px-7 py-4 text-sm font-bold text-white transition hover:bg-[#073B4C]"
          >
